...
exec devbox run --pure -e ANDROID_SKIP_DOWNLOADS=1 -e TEST_TUI="${TEST_TUI:-false}" bash -c 'process-compose -f tests/test-suite-ios-e2e.yaml --no-server --tui="${TEST_TUI:-false}"'
