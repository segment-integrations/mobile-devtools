{
  ...
  outputs =
    ...
    let
      ...
      # The lock file is in devbox.d/android/, read via ANDROID_CONFIG_DIR env var
      # On first initialization, android.lock may not exist yet, so provide defaults
      configDir = builtins.getEnv "ANDROID_CONFIG_DIR";
      androidLockPath = if configDir != "" then "${configDir}/android.lock" else ./android.lock;
      androidLockExists = builtins.pathExists androidLockPath;
      androidLockData = if androidLockExists
        then builtins.fromJSON (builtins.readFile androidLockPath)
        else {
