#!/usr/bin/env bash
# iOS Plugin - Shared Doctor Library
...
if [ "${IOS_DOCTOR_LIB_LOADED:-}" = "1" ]; then
  return 0
fi
IOS_DOCTOR_LIB_LOADED=1
...
doctor_resolve_config_dir() {
  ...
  for candidate in \
    "./devbox.d/ios" \
    "./devbox.d/segment-integrations.devbox-plugins.ios" \
    "./.devbox/virtenv/ios"; do
    if [ -d "$candidate" ]; then
      echo "$candidate"
      return 0
    fi
